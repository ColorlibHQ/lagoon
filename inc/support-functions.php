...
// Post Category
function lagoonlogistics_post_cats() {
...
// Post Tags
function lagoonlogistics_post_tags(){
...
// social media
if ( ! function_exists( 'lagoonlogistics_social' ) ) {
	function lagoonlogistics_social( $args = array()  ){
		...
		$url = lagoonlogistics_opt('lagoonlogistics_social_url');
...
function lagoonlogistics_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'lagoonlogistics' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'lagoonlogistics' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'lagoonlogistics' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function lagoonlogistics_set_post_views($postID) {
    $count_key = 'lagoonlogistics_post_views_count';
...
// blog post categoty 
function lagoonlogistics_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'lagoonlogistics' ) );
